...
class User {
    ...
    incProgress(activityRank){
      ...
      if(ranks.indexOf(activityRank) < 0){
        throw new Error("Rank does not exist");
      }else{
        let rankDiff = ranks.indexOf(activityRank) - ranks.indexOf(this.rank)
        let progressEarned = 0;
        if(rankDiff > -2){
          switch(rankDiff){
            case -1: progressEarned = 1;
            break;
            case 0: progressEarned = 3;
            break;
            default: progressEarned = 10 * rankDiff * rankDiff
            break;
          }
        }
