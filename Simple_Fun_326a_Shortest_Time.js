...
function shorterestTime(n,m,speeds) {
  ...
  if(n===0)return 0;
  let walk = n * speeds[3];
  let elevate = Math.abs(n-m)*speeds[0] + n * speeds[0] + speeds[1]*2 + speeds[2];
  let combo = Math.abs(n-m)*speeds[3] + m*speeds[0] + speeds[1]*2 + speeds[2];

  console.log(n,m, walk, elevate, combo);

  return (Math.min(walk, elevate, combo));
}
