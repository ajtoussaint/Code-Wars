...
function boxesPacking(length, width, height) {
  ...
  for(let i =0; i<length.length; i++){
    let obj = {}
    obj.volume = length[i] * width[i] * height[i];
    obj.sides = [length[i],width[i],height[i]];
    obj.sides = obj.sides.sort( (a,b) => {
      return b - a;
    })
    boxes.push(obj);
  }
  ...
  for(let j = 0; j < boxes.length-1; j++){
    if(boxes[j].sides[0] <= boxes[j+1].sides[0] || boxes[j].sides[1] <= boxes[j+1].sides[1] || boxes[j].sides[2] <= boxes[j+1].sides[2]){
      return false;
    }
  }
  return true;
